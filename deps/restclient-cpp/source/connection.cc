...
RestClient::Connection::AppendHeader(const std::string& key,
  ...
  this->headerFields.insert({key, value});
}
...
RestClient::Connection::GetUserAgent() {
  ...
  {
    return std::string("restclient-cpp/" RESTCLIENT_VERSION);
  }
}
...
/**
 * @brief set SSL hostname verification flag. (true means 2, false means 0)
 *
 * @param boolean (default is true)
 *
 */
...
RestClient::Connection::post(const std::string& url,
  ...
  return post(url, (char*)&data.front(), data.size());
}
/**
 * @brief HTTP POST method
 *
 * @param url to query
 * @param data_ptr HTTP POST body pointer
 * @param data_size HTTP POST body size
 *
 * @return response struct
 */
RestClient::Response
RestClient::Connection::post(const std::string& url,
  ...
  /** set post fields */
  curl_easy_setopt(getCurlHandle(), CURLOPT_POSTFIELDS, data_ptr);
  curl_easy_setopt(getCurlHandle(), CURLOPT_POSTFIELDSIZE, data_size);
...
RestClient::Connection::put(const std::string& url,
                            const std::string& data) {
  return put(url, data.c_str(), data.size());
}
/**
 * @brief HTTP PUT method
 *
...
RestClient::Connection::put(const std::string& url,
                            const std::vector<uint8_t>& data) {
  return put(url, (char*)&data.front(), data.size());
}
/**
 * @brief HTTP PUT method
 *
 * @param url to query
 * @param data_ptr HTTP PUT body pointer
 * @param data_size HTTP PUT body size
 *
 * @return response struct
 */
RestClient::Response
RestClient::Connection::put(const std::string& url,
  ...
  up_obj.data = data_ptr;
  up_obj.length = data_size;
...
RestClient::Connection::patch(const std::string& url,
                             const std::string& data) {
  return patch(url, data.c_str(), data.size());
}
/**
 * @brief HTTP PATCH method
 *
...
RestClient::Connection::patch(const std::string& url,
                             const std::vector<uint8_t>& data) {
  return patch(url, (char*)&data.front(), data.size());
}
/**
 * @brief HTTP PATCH method
 *
 * @param url to query
 * @param data_ptr HTTP PATCH body pointer
 * @param data_size HTTP PATCH body size
 *
 * @return response struct
 */
RestClient::Response
...
/**
 * @brief Add debug_callback to cactch logs
 *
 * @param value the logger
 */
void RestClient::Connection::SetLogger(const std::shared_ptr<Logger>& value)
{
    logger = value;

    if (logger != nullptr)
    {
        curl_easy_setopt(curlHandle, CURLOPT_DEBUGFUNCTION, RestClient::Connection::debug_callback);
        curl_easy_setopt(curlHandle, CURLOPT_DEBUGDATA, logger.get());
        curl_easy_setopt(curlHandle, CURLOPT_VERBOSE, 1L);
    }
    else
    {
        curl_easy_setopt(curlHandle, CURLOPT_DEBUGFUNCTION, nullptr);
        curl_easy_setopt(curlHandle, CURLOPT_DEBUGDATA, nullptr);
        curl_easy_setopt(curlHandle, CURLOPT_VERBOSE, 0L);
    }
}

/**
 * @brief Low level debug_callback as defined by libcurl
 *
 * @see https://curl.se/libcurl/c/CURLOPT_DEBUGFUNCTION.html
 */
int RestClient::Connection::debug_callback(CURL *handle, curl_infotype type, char *data, size_t size, void *userptr)
{
    Logger* logger = (Logger*)userptr;
